    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {!isAdding && (<Button onClick={() => setIsAdding(true)} size="sm" className="w-full sm:w-auto"><Plus className="w-4 h-4 mr-1" />Add Award</Button>)}
        </div>
      </CardHeader>
      <CardContent>
        {isAdding ? (
          <form onSubmit={handleSubmit} className="space-y-4 border p-4 rounded-lg bg-gray-50">
            ...
            <div className="flex flex-col gap-2 sm:flex-row">
              <Button type="submit" disabled={saving} className="w-full sm:w-auto">{saving ? 'Saving...' : editingId ? 'Update' : 'Add Award'}</Button>
          ...
          <div className="text-center py-12 border-2 border-dashed rounded-lg">
            ...
            <Button onClick={() => setIsAdding(true)} className="w-full sm:w-auto"><Plus className="w-4 h-4 mr-1" />Add Your First Award</Button>
          </div>
        ) : (
