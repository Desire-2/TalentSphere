...
Handles all external API communications (Gemini, OpenRouter) with retry logic and fallback
"""
import os
import time
import re
...
class CVAPIClient:
    ...
    def __init__(self):
        ...
        self._min_request_interval = 3  # Increased to 3 seconds between requests
    ...
    def _extract_retry_delay(self, error_message: str) -> int:
        ...
        # Try to extract "retry after X seconds/minutes"
        patterns = [
    ...
    def call_openrouter(self, prompt: str, temperature: float = 0.7, max_tokens: int = 2048) -> str:
        ...
        if 'choices' in result and len(result['choices']) > 0:
            ...
            print(f"[CV API] ✅ OpenRouter API successful")
    ...
    def make_request_with_retry(self, prompt: str, max_retries: int = 3) -> str:
        """
        Make API request with exponential backoff retry logic and automatic fallback
        ...
        # If OpenRouter quota is exhausted, use Gemini directly
        if self._openrouter_quota_exhausted and self._api_key:
            ...
            try:
                ...
                print(f"[CV API] ✅ Gemini successful")
                return response
            except Exception as e:
                print(f"[CV API] ⚠️ Gemini failed, retrying OpenRouter: {str(e)[:100]}")
                self._openrouter_quota_exhausted = False
                ...
                try:
                    ...
                    if is_rate_limit:
                        if attempt < max_retries - 1:
                            ...
                            print(f"[CV API] ⚠️ OpenRouter rate limit exhausted")
                            break
                    else:
                        print(f"[CV API] ⚠️ OpenRouter error: {error_str[:150]}")
                        if attempt < max_retries - 1:
                            time.sleep(2)
                            continue
                        else:
            ...
            try:
                ...
                if is_rate_limit:
                    if attempt < max_retries - 1:
                        ...
                        raise Exception(
                            "Both OpenRouter and Gemini API rate limits exceeded. "
