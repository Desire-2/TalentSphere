...
const MembershipsSection = ({ data = [], onUpdate }) => {
  ...
  const [formData, setFormData] = useState({ organization_name: '', role: '', start_date: '', end_date: '', is_current: true });
  const [saving, setSaving] = useState(false);

  const resetForm = () => { setFormData({ organization_name: '', role: '', start_date: '', end_date: '', is_current: true }); setIsAdding(false); setEditingId(null); };
  const handleEdit = (mem) => { setFormData({ organization_name: mem.organization_name || '', role: mem.role || '', start_date: mem.start_date || '', end_date: mem.end_date || '', is_current: mem.is_current || true }); setEditingId(mem.id); setIsAdding(true); };
        ...
        {isAdding ? (
          <form onSubmit={handleSubmit} className="space-y-4 border p-4 rounded-lg bg-gray-50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div><Label htmlFor="role">Role</Label><Input id="role" value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value})} placeholder="e.g., Member, Board Member" /></div>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {data.map((mem) => (
              <div key={mem.id} className="flex items-center justify-between p-3 border rounded group hover:bg-gray-50 transition-colors">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600">{mem.role || 'Member'} • Since {formatDate(mem.start_date)}</p>
                </div>
