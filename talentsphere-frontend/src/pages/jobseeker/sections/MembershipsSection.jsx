    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {!isAdding && (<Button onClick={() => setIsAdding(true)} size="sm" className="w-full sm:w-auto"><Plus className="w-4 h-4 mr-1" />Add Membership</Button>)}
        </div>
      </CardHeader>
      <CardContent>
        {isAdding ? (
          ...
          <div className="text-center py-12 border-2 border-dashed rounded-lg"><Users className="w-12 h-12 mx-auto text-gray-400 mb-3" /><p className="text-gray-500 mb-4">No memberships yet</p><Button onClick={() => setIsAdding(true)} className="w-full sm:w-auto"><Plus className="w-4 h-4 mr-1" />Add Membership</Button></div>
        ) : (
          <div className="space-y-3">
            {data.map((mem) => (
              <div key={mem.id} className="flex flex-col gap-2 p-3 border rounded group hover:bg-gray-50 transition-colors sm:flex-row sm:items-center sm:justify-between">
